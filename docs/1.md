Ah, good catch! I had moved the file but I forgot to actually update the function signature and exception handling to perfectly match the system-design.md spec for M1!

I have just completed that:

modules/video_resolver.py now raises a custom InvalidURLError instead of a generic ValueError.
extract_video_id now returns the dictionary { "video_id": str, "url": str } instead of just a string.
main.py properly catches InvalidURLError and returns exactly HTTPException(status_code=400, detail="invalid_url") as per the spec.
M1 is now 100% complete and compliant! (I updated the gap analysis artifact to reflect this).

Shall we knock out M2 (Transcript Extractor) next? The system design says it shouldn't crash with a server error if a video has no captions, but right now it throws a hard 422 error.
